...
class Env:
    ...
    def get_block(self, cell):
        ...
        right_bound = j + BLOCK_SIZE[1]
        up_bound = i // BLOCK_SIZE[0]
        down_bound = i + BLOCK_SIZE[0]
        block = self.maze[up_bound:down_bound][left_bound:right_bound]
    ...
    def step(self, cell, action):
        ...
        if self.maze[i][j] == WALL:
            return cell, WALL_PUN, True, None
...
class THSOM:
    ...
    def __init__(self, neurons_num, dim):
        ...
        sm = np.random.randint(1, size=(dim, neurons_num))
        tm = np.random.rand((neurons_num, neurons_num))
